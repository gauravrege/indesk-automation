...
Built a Node tool that reads airline GST invoices out of a folder and writes every figure on them into one Excel workbook. Thirteen airlines are supported, in PDF and in HTML, and each one prints its invoice in a different layout. Typing a batch of these into the GST sheet by hand took an afternoon; the tool does 141 files in under two seconds.
...
- **A Regression Test With Teeth:** 139 real invoices covering all thirteen airlines are kept as a local test set, never committed. Re-parsing them must report 156 documents and exactly three known exceptions; anything else in that list is something the change broke.
